Shader "Abyss/HexagonalPatternShader_Controlled"
{
    Properties
    {
        ...
        _BaseColor ("Base Color", Color) = (0.05098, 0.25098, 0.2784, 1.0)
        _BorderColor ("Border Color", Color) = (0.0, 0.0, 0.0, 1.0)
        _IsolineColor1 ("Isoline Color 1", Color) = (0.0, 0.1, 0.09, 1.0)
        _IsolineColor2 ("Isoline Color 2", Color) = (0.1, 0.2, 0.3, 1.0)
    }
    SubShader
    {
        Pass
        {
            ...
            float4 _BaseColor;
            float4 _BorderColor;
            float4 _IsolineColor1;
            float4 _IsolineColor2;
...
#define S(r, v) smoothstep(9./iResolution.y, 0., abs(v-r))
            ...
            float4 frag (v2f __vertex_output) : SV_Target
            {
                ...
                float3 borderCol = _BorderColor.rgb;
                float3 isoline1Col = _IsolineColor1.rgb;
                float3 isoline2Col = _IsolineColor2.rgb;

                float3 isoline = 
                    S(isolineOffset, totalDist) * isoline1Col * step(3. + rand * 0.5, glsl_mod(angle, 6.28)) + 
                    S(isolineOffset2, totalDist) * isoline2Col * step(4. + rand * 1.5, glsl_mod(angle + rand * 2., 6.28));

                fragColor.rgb = 
                    (smoothstep(0.51, 0.51 - aa, totalDist) + 
                    pow(1. - max(0., 0.5 - totalDist), 20.) * 1.5) * 
                    (baseCol + rand * borderCol) + isoline + 
                    baseCol * smoothstep(0.2 + sinOffset, 0.2 + sinOffset - aa, totalDist);
